...
madx.input('SELECT, FLAG = ERROR, PATTERN = "qq2a.2";    EALIGN, DX = 0.00005*TGAUSS(2.5), DY = 0.00005*TGAUSS(2.5),DS = 0.00005*TGAUSS(2.5);')
madx.input('SELECT, FLAG = ERROR, PATTERN = "qq2b.2";    EALIGN, DX = 0.00005*TGAUSS(2.5), DY = 0.00005*TGAUSS(2.5),DS = 0.00005*TGAUSS(2.5);')
madx.input('SELECT, FLAG = ERROR, PATTERN = "qq1a.3";    EALIGN, DX = 0.00005*TGAUSS(2.5), DY = 0.00005*TGAUSS(2.5),DS = 0.00005*TGAUSS(2.5);')
madx.input('SELECT, FLAG = ERROR, PATTERN = "qq1b.3";    EALIGN, DX = 0.00005*TGAUSS(2.5), DY = 0.00005*TGAUSS(2.5),DS = 0.00005*TGAUSS(2.5);')
madx.input('SELECT, FLAG = ERROR, PATTERN = "qq2a.4";    EALIGN, DX = 0.00005*TGAUSS(2.5), DY = 0.00005*TGAUSS(2.5),DS = 0.00005*TGAUSS(2.5);')
madx.input('SELECT, FLAG = ERROR, PATTERN = "qq2b.4";    EALIGN, DX= 0.00005*TGAUSS(2.5), DY = 0.00005*TGAUSS(2.5),DS = 0.00005*TGAUSS(2.5);')


madx.input('EOPTION, SEED = 100, ADD=True;')

madx.input('SELECT, FLAG = ERROR, PATTERN = "xs1.1";    EALIGN, DX = 0.00005*TGAUSS(2.5), DY = 0.00005*TGAUSS(2.5),DS = 0.00005*TGAUSS(2.5);')
...
init_amp_x=np.linspace(0,sig_10x,4)#sig_2x,3) #going from 0 to 2sigma for 3 steps in x
init_amp_y=np.linspace(0,sig_10y,4)
...
for i in range(len(init_amp_y)):
    for j in range(len(init_amp_x)):
        ...
        ptc_track, icase=4, closed_orbit,dump,MAXAPER={0.03,0.01,0.03,0.01},turns=2047,ffile=1,
        NORM_OUT;
        ptc_track_end;
        ptc_end;
        '''%(s_x,s_y))
...
for i in range(len(myString)):
    ...
    if len(x[i])<12:
        ...
        var = (np.empty(12 - len(x[i]), dtype=object))
        var.fill(np.nan)
        dum1.extend(var)
        x[i].extend(dum1)
    ...
    if len(y[i])<12:
        var = (np.empty(12 - len(y[i]), dtype=object))
        var.fill(np.nan)
        dum2.extend(var)
        y[i].extend(dum2)
    ...
    if len(px[i])<12:
        var = (np.empty(12 - len(px[i]), dtype=object))
        var.fill(np.nan)
        dump1.extend(var)
        px[i].extend(dum1)
        #print('var=',dum1) #var is fine
    ...
    if len(py[i])<12:
        var = (np.empty(12 - len(py[i]), dtype=object))
        var.fill(np.nan)
        dump2.extend(var)
        py[i].extend(dum2)
        #print('var=',dum2) #var is fine
    ...
    #tune plots
    myTunesPTC.append(np.linspace(0, 1, 12))
    ...
    freq_x_1024.append(NAFFlib.get_tune(np.array(x[i][0:6])))  # this gets the fft
    freq_y_1024.append(NAFFlib.get_tune(np.array(y[i][0:6])))
    freq_x_2048.append(NAFFlib.get_tune(np.array(x[i][6:12])))
    freq_y_2048.append(NAFFlib.get_tune(np.array(y[i][6:12])))
...
with open("1009_1024_xtunes_sbenderr_tunedmulti_nat.txt","w") as external_file:
    print(*q_x_1024,sep='\n',file=external_file)
external_file.close()
with open("1009_1024_ytunes_sbenderr_tunedmulti_nat.txt","w") as external_file:
    print(*q_y_1024,sep='\n',file=external_file)
external_file.close()
...
with open("1009_jx_matched_sbenderr_tunedmulti_nat.txt","w") as external_file:
    print(*Jx[:,0],sep='\n',file=external_file)
external_file.close()
with open("1009_jy_matched_sbenderr_tunedmulti_nat.txt","w") as external_file:
    print(*Jy[:,0],sep='\n',file=external_file)
external_file.close()
