...
def plot_lattice_elements(ax, twiss_in, suppress_x=True):
    ...
    if (twissHeader['SEQUENCE'] == 'TERARING'):
    ...
    for i in idx_1:
        ax.add_patch(
            Rectangle((pos[i], (-0.5 + offset[0])), width=lengths[i], height=1., angle=0.0, ec='k', fc='r', lw=0.0))
    for i in idx_2:
        ax.add_patch(
            Rectangle((pos[i], (-0.5 + offset[1])), width=lengths[i], height=1., angle=0.0, ec='k', fc='r', lw=0.0))
