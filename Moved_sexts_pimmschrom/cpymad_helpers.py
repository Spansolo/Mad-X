...
def cpymad_savebeta(madx_instance, cpymad_logfile, sequence_name, savebeta_label, location):
    ...
    madx_command = 'savebeta, label=' + str(savebeta_label) + ', place=' + str(location) + ', sequence=' + str(
        sequence_name) + ';'
...
def cpymad_make_ps_plot(madx_instance, cpymad_logfile, sequence_name):
    ...
    madx_instance.input(
        'plot, table=twiss, haxis=s, vaxis1=betx,bety,dx, VAXIS2=mux,muy, vmin=0,vmax=30., interpolate, title=' + str(
            sequence_name) + ', style=100, file=plot;')
...
def cpymad_save_sequence(madx_instance, cpymad_logfile, sequence_name, savename, beam=False, bare=False, mad8=False):
    ...
    cpymad_write_to_logfile(cpymad_logfile, log_string)
    madx_instance.input(str(
        'save, sequence=' + sequence_name + ', file=' + savename + ', beam=' + str(beam) + ', bare=' + str(
            bare) + ', mad8=' + str(mad8) + ';'))
...
def cpymad_load_sequence_from_file(madx_instance, cpymad_logfile, sequence_file):
    ...
    for line in file_lines:
        if ': sequence,' in line:
            ...
            seq_count += 1
    if seq_count >= 2: print(
        'cpymad_load_sequence_from_file::warning: multiple sequences in input file, first selected is ', stored_name)
...
########################################################################
def cpymad_insert_element(madx_instance, cpymad_logfile, sequence_name, cpymad_element, cpymad_class, cpymad_at,
                          cpymad_from=None):
    ...
    madx_instance.input(str('seqedit, sequence=' + sequence_name + ';'))
    madx_instance.input('flatten;')
    if cpymad_from is None:
        madx_instance.input(
            'INSTALL, ELEMENT=' + str(cpymad_element) + ', CLASS=' + str(cpymad_class) + ', AT=' + str(cpymad_at) + ';')
    else:
        madx_instance.input('INSTALL, ELEMENT=' + str(cpymad_element) + ', CLASS=' + str(cpymad_class) + ', AT=' + str(
            cpymad_at) + ', FROM=' + str(cpymad_from) + ';')
    madx_instance.input('flatten;')
    madx_instance.input('endedit;')
...
def cpymad_print_element_from_sequence(madx_instance, cpymad_logfile, cpymad_sequence, element):
    if element in cpymad_sequence.elements:
        ...
        print('cpymad_print_element_from_sequence:: element', element, ' not present in sequence ',
              cpymad_sequence.name)
        return False
...
def cpymad_return_element_from_sequence(madx_instance, cpymad_logfile, cpymad_sequence, element):
    if element in cpymad_sequence.elements:
        return str(cpymad_sequence.elements[element])
    else:
        print('cpymad_print_element_from_sequence:: element', element, ' not present in sequence ',
              cpymad_sequence.name)
...
def cpymad_return_global_element(madx_instance, cpymad_logfile, element):
    if element in madx_instance.elements:
        return str(madx_instance.elements[element])
    else:
        print('cpymad_print_global_element:: element', element, ' not present in global MAD-X variables')
...
def cpymad_return_detailed_element(madx_instance, cpymad_logfile, element_name):
    if element_name in madx_instance.elements:
        ...
        if element_class == ('sbend'):
            ...
            bend = str(element_name + ' : ' + element_class + ', l=' + str(
                madx_instance.elements[element_name].defs.l) + ', angle=' + str(
                madx_instance.elements[element_name].defs.angle) + ', k1=' + str(
                madx_instance.elements[element_name].defs.k1) + ', e1=' + str(
                madx_instance.elements[element_name].defs.e1) + ', e2=' + str(
                madx_instance.elements[element_name].defs.e2) + ', fint=' + str(
                madx_instance.elements[element_name].defs.fint) + ', fintx=' + str(
                madx_instance.elements[element_name].defs.fintx) + ';')
            # print(bend)
            return bend
            ...
            # store k1 variable
            quad = str(element_name + ' : ' + element_class + ', l=' + str(
                madx_instance.elements[element_name].defs.l) + ', k1=' + str(
                madx_instance.elements[element_name].defs.k1) + ';')
            return quad

        elif element_class == ('sextupole'):
            # store k2 variable
            sext = str(element_name + ' : ' + element_class + ', l=' + str(
                madx_instance.elements[element_name].defs.l) + ', k2=' + str(
                madx_instance.elements[element_name].defs.k2) + ';')
            return sext

        elif element_class == ('kicker'):
            kicker = str(element_name + ' : ' + element_class + ', kick=' + str(
                madx_instance.elements[element_name].defs.kick) + ';')
            return kicker
            ...
            drift = str(
                element_name + ' : ' + element_class + ', l=' + str(madx_instance.elements[element_name].defs.l) + ';')
            return drift
...
def cpymad_rename_element_in_sequence(madx_instance, cpymad_logfile, cpymad_sequence, old_element, new_element):
    ...
    if old_element in cpymad_sequence.elements:
        ...
        duplicate_element = cpymad_create_complete_element_duplicate(madx_instance, cpymad_logfile, old_element,
                                                                     new_element)

        # replace the element
        cpymad_replace_element_in_sequence(madx_instance, cpymad_logfile, cpymad_sequence.name, old_element,
                                           duplicate_element)

    else:
        print('cpymad_rename_element_in_sequence:: Requested element', old_element, ' not present in sequence ',
              cpymad_sequence.name)
...
def cpymad_extract_sequence(madx_instance, cpymad_logfile, sequence_name, new_sequence_name, seq_start, seq_end):
    ...
    madx_instance.input(str('seqedit, sequence=' + sequence_name + ';'))
    madx_instance.input('flatten;')
    madx_instance.input(str('extract, sequence=' + sequence_name + ', from=' + str(seq_start) + ', to=' + str(
        seq_end) + ', newname=' + new_sequence_name + ';'))
    madx_instance.input('flatten;')
    madx_instance.input('endedit;')
...
########################################################################
def cpymad_individual_element_naming_in_sequence(madx_instance, cpymad_logfile, cpymad_sequence, repeated_element,
                                                 new_name_start=None, verbose=True):
    ...
    for element in cpymad_sequence.elements:
        ...
        if element.name == repeated_element:
            ...
            new_element = cpymad_create_complete_element_duplicate(madx_instance, cpymad_logfile, repeated_element,
                                                                   str(new_name_start + '_' + str(count_iterator)))
            if verbose: print('Duplicated to: ',
                              cpymad_return_detailed_element(madx_instance, cpymad_logfile, new_element))

            # replace
            cpymad_replace_element_in_sequence(madx_instance, cpymad_logfile, cpymad_sequence.name, repeated_element,
                                               new_element)
...
def find_element_class(cpymad_element):
    elements = ['quadrupole', 'sbend', 'marker', 'hkicker', 'vkicker', 'kicker', 'drift', 'marker', 'rbend',
                'sextupole', 'octupole', 'multipole', 'solenoid', 'tkicker', 'rfcavity', 'rfmultipole', 'crabcavity',
                'elseparator', 'monitor', 'hmonitor', 'vmonitor', 'instrument', 'placeholder', 'collimator',
                'ecollimator', 'rcollimator', 'beambeam', 'matrix', 'yrotation', 'srotation', 'translation',
                'changeref']
...
########################################################################
def cpymad_replace_two_elements_with_element(madx_instance, cpymad_logfile, madx_sequence, element_list,
                                             new_madx_element):
    ...
    for i in range(len(sequence_list)):
        if (element_list[0] in sequence_list[i]) and (element_list[1] in sequence_list[i + 1]):
            print('cpymad_replace_two_elements_by_element: elements ' + sequence_list[i] + ' and ' + sequence_list[
                i + 1] + ' replaced with ' + new_madx_element.name)

            # Insert new_element_name
            cpymad_insert_element(madx_instance, cpymad_logfile, madx_sequence.name, new_madx_element.name,
                                  find_element_class(new_madx_element), '0.0', sequence_list[i])
            ...
            log_string += '\n! elements ' + sequence_list[i] + ' and ' + sequence_list[
                i + 1] + ' replaced with ' + new_madx_element.name
...
def cpymad_save_detailed_line(madx_instance, cpymad_logfile, madx_sequence, save_name):
    ...
    ignore_globals = ['version', 'pi', 'twopi', 'degrad', 'raddeg', 'e', 'amu0', 'emass', 'mumass', 'nmass', 'pmass',
                      'clight', 'qelect', 'hbar', 'erad', 'prad', 'none', 'twiss_tol']
...
def block_diagram(ax1, df_myTwiss, ptc_twiss=False):
    ...
    for i in range(len(DF)):
        aux = DF.iloc[i]
        ax1.add_patch(
            patches.Rectangle((DF.iloc[i].s - DF.iloc[i].l, 0.), DF.iloc[i].l, 1.0, color='green', alpha=0.5));
...
def cpymad_plot_CO(madx_instance, df_myTwiss, sequence_name, save_file, ptc_twiss=False, beta_rel=None):
    ...
    if np.min(df_myTwiss['bety']) < np.min(df_myTwiss['betx']):
        bet_min = round_down_n(np.min(df_myTwiss['bety']), 5)
    else:
        bet_min = round_down_n(np.min(df_myTwiss['betx']), 5)
    if np.max(df_myTwiss['bety']) > np.max(df_myTwiss['betx']):
        bet_max = round_up_n(np.max(df_myTwiss['bety']), 10)
    else:
        bet_max = round_up_n(np.max(df_myTwiss['betx']), 10)
    ...
    if np.min(df_myTwiss[key_dy]) < np.min(df_myTwiss[key_dx]):
        d_min = round_down_n(np.min(df_myTwiss[key_dy]), 1)
    else:
        d_min = round_down_n(np.min(df_myTwiss[key_dx]), 1)
    if np.max(df_myTwiss[key_dy]) > np.max(df_myTwiss[key_dx]):
        d_max = round_up_n(np.max(df_myTwiss[key_dy]), 10)
    else:
        d_max = round_up_n(np.max(df_myTwiss[key_dx]), 10)
    ...
    if np.min(df_myTwiss['y']) < np.min(df_myTwiss['x']):
        co_min = round_down_n(np.min(df_myTwiss['y']), 10)
    else:
        co_min = round_down_n(np.min(df_myTwiss['x']), 10)
    if np.max(df_myTwiss['y']) > np.max(df_myTwiss['x']):
        co_max = round_up_n(np.max(df_myTwiss['y']), 10)
    else:
        co_max = round_up_n(np.max(df_myTwiss['x']), 10)
...
########################################################################
def cpymad_plot_CO_correction(madx_instance, df_myTwiss, df_myTwiss2, sequence_name, save_file, ptc_twiss=False,
                              beta_rel=None):
    if ptc_twiss:
        if beta_rel == None:
            print('cpymad_plot_CO: Error: ptc_twiss selected but no beta_rel supplied for dispersion normalisation')
...
def cpymad_plot_madx_twiss(madx_instance, df_myTwiss, title=None, savename=None, use_caps=False):
    ...
    if use_caps is False:
        try:
            ...
            try:
                ...
                print('cpymad_plotTwiss::unkown bug')
    else:
        plt.plot(df_myTwiss['S'], 0 * df_myTwiss['S'], 'k')
    ...
    if np.min(df_myTwiss[key_bety]) < np.min(df_myTwiss[key_betx]):
        bet_min = round_down_10(np.min(df_myTwiss[key_bety]))
    else:
        bet_min = round_down_10(np.min(df_myTwiss[key_betx]))
    if np.max(df_myTwiss[key_bety]) > np.max(df_myTwiss[key_betx]):
        bet_max = round_up_10(np.max(df_myTwiss[key_bety]))
    else:
        bet_max = round_up_10(np.max(df_myTwiss[key_betx]))
    ...
    if np.min(df_myTwiss[key_dy]) < np.min(df_myTwiss[key_dx]):
        d_min = round_down(np.min(df_myTwiss[key_dy]))
    else:
        d_min = round_down(np.min(df_myTwiss[key_dx]))
    if np.max(df_myTwiss[key_dy]) > np.max(df_myTwiss[key_dx]):
        d_max = round_up_10(np.max(df_myTwiss[key_dy]))
    else:
        d_max = round_up_10(np.max(df_myTwiss[key_dx]))
...
def cpymad_plot_ptc_twiss_block(madx_instance, df_myTwiss, title=None, savename=None, use_caps=False):
    ...
    for i in range(len(DF)):
        ...
        if use_caps:
            ax1.add_patch(
                patches.Rectangle((DF.iloc[i].S - element_length, 0.), element_length, 1.0, color='r', alpha=0.5))
        else:
            ax1.add_patch(
                patches.Rectangle((DF.iloc[i].s - element_length, 0.), element_length, 1.0, color='r', alpha=0.5))
    ...
    for i in range(len(DF)):
        ...
        if use_caps:
            ax1.add_patch(
                patches.Rectangle((DF.iloc[i].S - element_length, 0.), element_length, 1.0, color='green', alpha=0.5))
        else:
            ax1.add_patch(
                patches.Rectangle((DF.iloc[i].s - element_length, 0.), element_length, 1.0, color='green', alpha=0.5))
    ...
    for i in range(len(DF)):
        ...
        if use_caps:
            ax1.add_patch(
                patches.Rectangle((DF.iloc[i].S - element_length, 0.), element_length, 1.0, color='b', alpha=0.5))
        else:
            ax1.add_patch(
                patches.Rectangle((DF.iloc[i].s - element_length, 0.), element_length, 1.0, color='b', alpha=0.5))
    ...
    for i in range(len(DF)):
        ...
        if use_caps:
            ax1.add_patch(
                patches.Rectangle((DF.iloc[i].S - element_length, 0.), element_length, kicker_height, color='c',
                                  alpha=0.5))
        else:
            ax1.add_patch(
                patches.Rectangle((DF.iloc[i].s - element_length, 0.), element_length, kicker_height, color='c',
