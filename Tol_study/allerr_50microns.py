...
madx.input('SELECT, FLAG = ERROR, PATTERN = "mb.*";    EALIGN, DX:= 0.0001*TGAUSS(2.5), DY:= 0.0001*TGAUSS(2.5),DS :=0.0001*TGAUSS(2.5)')


madx.input('SELECT, FLAG = ERROR, PATTERN = "qq.*";    EALIGN, DX := 0.00005*TGAUSS(2.5), DY := 0.00005*TGAUSS(2.5),DS := 0.00005*TGAUSS(2.5)')


madx.input('SELECT, FLAG = ERROR, PATTERN = "xs.*";    EALIGN, DX := 0.00005*TGAUSS(2.5), DY := 0.00005*TGAUSS(2.5),DS := 0.00005*TGAUSS(2.5)')
...
for i in range(len(init_amp_y)):
    for j in range(len(init_amp_x)):
        ...
        ptc_track, icase=4, closed_orbit,dump,turns=2047,ffile=1,
        NORM_OUT;
        ptc_track_end;
        ptc_end;
        '''%(s_x,s_y))
...
with open("1807_1024_xtunes_alltuned_matched_multi_nat.txt","w") as external_file:
    print(*q_x_1024,sep='\n',file=external_file)
external_file.close()
with open("1807_1024_ytunes_alltuned_matched_multi_nat.txt","w") as external_file:
    print(*q_y_1024,sep='\n',file=external_file)
external_file.close()

with open("1807_2048_xtunes_alltuned_matched_multi_nat.txt","w") as external_file:
    print(*q_x_2048,sep='\n',file=external_file)
external_file.close()
with open("1807_2048_ytunes_alltuned_matched_multi_nat.txt","w") as external_file:
    print(*q_y_2048,sep='\n',file=external_file)
external_file.close()
...
with open("1807_jx_matched_alltuned_multi_nat.txt","w") as external_file:
    print(*Jx[:,0],sep='\n',file=external_file)
external_file.close()
with open("1807_jy_matched_alltuned_multi_nat.txt","w") as external_file:
    print(*Jy[:,0],sep='\n',file=external_file)
external_file.close()
